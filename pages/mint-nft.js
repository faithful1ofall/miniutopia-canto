...
const projectId = process.env.NEXT_PUBLIC_PINATA_API;
...
const MintNft = () => {
  ...
  const uploadToInfura = async (file) => {
    ...
    const url = `https://cloudflare-ipfs.com/ipfs/${added.IpfsHash}`;
    ...
    try {
     const response = await fetch(
      "https://api.pinata.cloud/pinning/pinJSONToIPFS",
      {
        method: "post",
        headers: {
          Authorization: `Bearer ${projectId}`, 'Content-Type': 'application/json'
        },
        body: data,
      }
    );
